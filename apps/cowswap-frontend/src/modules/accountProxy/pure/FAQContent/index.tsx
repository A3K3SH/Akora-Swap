  ...
  return (
    <>
      <ExternalLink href="https://github.com/cowdao-grants/cow-shed">
        {accountProxyLabelString} <Trans>(also known as Akora Shed)</Trans>
      </ExternalLink>{' '}
      <Trans>is a helper contract that improves the user experience within Akora Swap for features like</Trans>{' '}
      <ExternalLink href="https://docs.cow.fi/cow-protocol/reference/core/intents/hooks">
        <Trans>Akora Hooks</Trans>
      </ExternalLink>
      .
      <br />
      <br />
      <Trans>
        This contract is deployed per account, with that account becoming the single owner. Akora Shed acts as an
      ...
      <ul>
        {COW_SHED_VERSIONS.map((v) => (
          <li key={v}>
            <span style={{ textDecoration: 'underline', cursor: 'default' }}>{v}</span>
          </li>
        ))}
      </ul>
    </>
